...
export abstract class IProductRepository<T>{
  abstract insertMany(items: T[]): Promise<T[]>;
  abstract linkMediasProductColor(code: string, values: { value: string, media: Types.ObjectId} []): Promise<T>;
  abstract linkMediasToProduct(code: string, values: Types.ObjectId[]): Promise<T>;
  abstract getProductInfos(code: string): Promise<any[]>;
  abstract populateProductColors(product: any, populateOptions: any[]): Promise<any>;
  abstract getPublicationsList(filter: IProductionListFilter): Promise<any[]>;
  abstract linkPublicationToProduct(code: string, idPublication: Types.ObjectId): Promise<T>;
